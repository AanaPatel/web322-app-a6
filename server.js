...
app.get("/student/:studentId", ensureLogin, (req, res) => {
  ...
  data.getStudentById(req.params.studentId).then((data) => {
    ...
    }).then(data.getPrograms).then((data) => {
...
app.use((req, res) => {
  res.status(404).send(
      "<h1>ERROR 404. PAGE NOT FOUND.</h1><img alt='Error! No Image can be displayed.'/>"
    );
});

data.initialize().then(dataServiceAuth.initialize()).then(() => {
